...
from structure_nclose import (
    MODEL_FILE, MODEL_VERSION, StructureWeights, add_bnd_metadata,
    add_virtual_structure, count_path_members, load_or_build_structure_model,
...
def fixture(structures):
    ...
    return nodes, dict(version=MODEL_VERSION, ncloses=ncloses, structures=structures,
                       column_count=len(structures), next_nclose_id=4)


def write_internal_chromosome_fixture(prefix):
    # 160 kb aligned in total; the internal chr2 chunk is 60 kb (37.5%).
    # This satisfied the former 100 kb / 20% / 10 kb split thresholds.
    nodes = [node("u1", 0, 50_000, "+", "chr1", 100_000, 150_000),
             node("u1", 50_000, 110_000, "+", "chr2", 200_000, 260_000),
             node("u1", 110_000, 160_000, "+", "chr3", 300_000, 350_000)]
    for row in nodes:
        row[1] = 160_000
    pair = (0, 2)
    path = [(0, "chr1f", 0, 0), (1, 0, 0, 0),
            (1, 2, 1, 0), (0, "chr3b", 1, 0)]
    path_file = prefix / "10_paths" / "chr1" / "1.txt"
    path_file.parent.mkdir(parents=True)
    path_file.write_text("\n".join(" ".join(map(str, step)) for step in path) + "\n")
    component_dir = prefix / "21_pat_depth"
    component_dir.mkdir()
    (component_dir / "1.paf").write_text("\n".join(
        "\t".join(map(str, row + [row[8] - row[7], row[8] - row[7], 60]))
        for row in nodes) + "\n")
    paths = [(str(path_file), [1])]
    int2key = {1: (1, ((1, 0), (1, 2)))}
    artifacts = {
        "nclose_event_catalog.pkl": [_build_bnd_event(pair, nodes, "compressed_nclose_nodes_list.txt")],
        "tot_loc_list.pkl": [str(path_file.with_suffix(".win.stat.gz"))],
        "contig_pat_vec_data.pkl": (paths, [], int2key, {}),
        "path_data.pkl": {"chr1": [(path,)]},
        "ecdna_circuit_data.pkl": ([],),
        "conjoined_type4_ins_del.pkl": ([], []),
        "01_nclose_data.pkl": {"contig_data": nodes},
    }
    for name, value in artifacts.items():
        with (prefix / name).open("wb") as handle:
            pickle.dump(value, handle)
    return nodes, pair, paths, int2key


class StructureWeightTests(unittest.TestCase):
    def test_stage31_internal_chromosome_keeps_original_endpoints_and_weight(self):
        # Execute the real native stage-31 accounting block without plots or fitting.
        source = Path(__file__).resolve().parents[1] / "31_depth_analysis.py"
        tree = ast.parse(source.read_text())
        block, = [statement for statement in tree.body if isinstance(statement, ast.If)
                  and any(isinstance(n, ast.Assign)
                          and any(isinstance(t, ast.Name) and t.id == "structure_model"
                                  for t in n.targets) for n in statement.body)]
        with tempfile.TemporaryDirectory() as tmp:
            ...
            namespace = dict(
                ...
                pipeline_input_config={}, pipeline_input_is_vcf=lambda config: False,
                load_or_build_structure_model=load_or_build_structure_model,
                build_virtual_inv_events=lambda *args, **kwargs: [],
            ...
            self.assertEqual([call["endpoints"] for call in calls], [
                ...
                (("chr2", 260_000, "L"), ("chr3", 300_000, "R")),
            ])
            self.assertEqual([call["weight"] for call in calls], [1.2, 1.2])
            self.assertEqual([call["info"]["SVCLASS"] for call in calls], [["NCLOSE"], ["NCLOSE"]])
            event, = namespace["native_display_events"]
            self.assertEqual(event["spans"], [("chr1", 100_000, 150_000), ("chr3", 300_000, 350_000)])
            self.assertEqual(event["link"], [("chr1", 100_000), ("chr3", 300_000)])
            self.assertEqual(event["weight"], 12.)
            self.assertEqual(event["source_id"], calls[0]["info"]["NCLOSE_IDS"][0])
            write_native_bed(tmp, [event])
            with (Path(tmp) / "SKYPE_result.bed").open() as handle:
                rows = list(csv.DictReader(handle, delimiter="\t"))
            self.assertEqual([r["#chrom"] for r in rows], ["chr1", "chr3"])
            self.assertEqual([float(r["weight (N)"]) for r in rows], [1.2, 1.2])
            self.assertEqual(weights, [12.])
            self.assertEqual(context.totals[pair], 12.)

    def test_old_split_cache_rebuilds_and_round_trips_without_split_metadata(self):
        for old_version in (1, 2):
            with self.subTest(version=old_version), tempfile.TemporaryDirectory() as tmp:
                nodes, pair, _, _ = write_internal_chromosome_fixture(Path(tmp))
                old = load_or_build_structure_model(tmp)
                old.update(version=old_version, path_splits=[dict(
                    projection_id="SKYPE.nclose.1.PATH_SPLIT.1", parent_key=pair,
                    endpoints=(("chr1", 150_000, "L"), ("chr2", 200_000, "R")),
                    column_counts={0: 1})], split_parent_usage={pair: {0: 1}})
                save_structure_model(tmp, old)
                rebuilt = load_or_build_structure_model(tmp)
                self.assertEqual(rebuilt["version"], 3)
                self.assertNotIn("path_splits", rebuilt)
                self.assertNotIn("split_parent_usage", rebuilt)
                context = StructureWeights(rebuilt, [12.], 10.)
                calls = bnd_calls(context, nodes)
                self.assertEqual([call["weight"] for call in calls], [1.2, 1.2])
                self.assertEqual([call["endpoints"] for call in calls], [
                    tuple(sorted(node_pair_endpoints(nodes, pair)[0])) for pair in ((0, 1), (1, 2))])
                save_structure_model(tmp, rebuilt)
                with (Path(tmp) / MODEL_FILE).open("rb") as handle:
                    self.assertEqual(pickle.load(handle), rebuilt)
                with patch("structure_nclose.build_structure_model", side_effect=AssertionError("unexpected rebuild")):
                    cached = load_or_build_structure_model(tmp)
                self.assertEqual(cached, rebuilt)
    ...
    def test_virtual_new_source_reuses_existing_fitted_nclose(self):
        ...
        call = next(c for c in bnd_calls(context, nodes) if PAIR in c["keys"])
    ...
    def test_threshold_applies_after_sum_in_all_nclose_views(self):
        ...
        self.assertEqual(len(bnd_calls(context, nodes)), 1)
        self.assertEqual(bnd_calls(context, nodes)[0]["weight"], .12)
    ...
    def test_exact_geometry_reuses_identity_and_preserves_both_occurrences(self):
        nodes, model = fixture([dict(kind="PATH", nclose_counts={PAIR: 1, OTHER: 1})])
    ...
    def test_compressed_identity_is_preferred_to_earlier_amp_catalog_entry(self):
        nodes, model = fixture([dict(kind="AMP", nclose_counts={PAIR: 1})])
        ...
        self.assertEqual(bnd_calls(context, nodes)[0]["info"]["NCLOSE_IDS"], ["SKYPE.nclose.2"])

    def test_all_source_contributions_remain_on_exact_alias(self):
        nodes, model = fixture([dict(kind="PATH", nclose_counts={PAIR: 1, OTHER: 1}),
        ...
        call, = bnd_calls(context, nodes)
        self.assertEqual(call["weight"], 4.)
        self.assertEqual(call["info"]["NCLOSE_IDS"], ["SKYPE.nclose.1"])
        self.assertEqual(call["info"]["NCLOSE_KEYS"], ["0:1", "2:3"])
        self.assertEqual(call["info"]["PARENT_MULTIPLICITY"], [2, 1])
        self.assertEqual(call["info"]["SVCLASS"], ["AMPLICON", "NCLOSE"])
    ...
    def test_repeated_path_and_amp_contributions_remain_on_original_nclose(self):
        nodes, model = fixture([dict(kind="PATH", nclose_counts={PAIR: 2}),
        ...
        self.assertEqual(call["weight"], 4.)
        self.assertEqual(call["info"]["PARENT_MULTIPLICITY"], [2, 1])
        self.assertEqual(context.visible_ncloses(), [(context.ncloses[PAIR], 40.)])
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with path.with_suffix(".bnd_weights.tsv").open() as handle:
                ...
                self.assertEqual(reader.fieldnames, [
                    "bnd_id", "feature_index", "occurrence_count", "feature_weight_N",
                    "contribution_N", "weight_method", "structure_id", "nclose_id",
                    "structure_kind", "source_nclose_key", "nclose_occurrence_count",
                    "bnd_occurrences_per_nclose", "junction_indices",
                ])
                rows = list(reader)
        deletion, = [r for r in records if r.INFO["SVTYPE"] == "DEL"]
        self.assertEqual(deletion.INFO["WEIGHT"], 1.)
        self.assertEqual(deletion.INFO["NCLOSE_IDS"], ["SKYPE.nclose.3"])
