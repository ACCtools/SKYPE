...
def import_data(file_path : str) -> list :
    ...
    for curr_contig in paf_file:
        contig_data.append(tuple(curr_contig.split("\t")[:9]))
    return contig_data
...
def main():
    ...
    PREPROCESSED_PAF_FILE_PATH = PAF_FILE_PATH + '.ppc.paf'

    contig_data = import_data(PAF_FILE_PATH)
    ...
    with open(PREPROCESSED_PAF_FILE_PATH, "wt") as processed_paf:
        for i in range(contig_data_size):
            if contig_data[i][CTG_NAM] in preprocess_result:
                ...
                processed_paf.write(str(idx) + "\n")
                cnt+=1
            idx+=1
                

    print(f"After Preeprocessing: {len(preprocess_result)} contigs, {cnt} nodes exist")
